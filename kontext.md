...
1. **√ñffentliche Website**  
   Seri√∂se Family-Office-Darstellung mit Fokus auf Bestand, Haltung und Service.

2. **Mieterportal**  
   Gesch√ºtzter Bereich f√ºr Mieter, sp√§ter auch als App-Oberfl√§che f√ºr iPhone und Android.

3. **Verwalterbereich**  
   Interner Bereich f√ºr Stammdaten, Zuordnungen und sp√§tere Verwaltungsprozesse.
...
- **Qualit√§tssicherung:** `npm run lint`

Wichtige Vorgabe:
- Vor √Ñnderungen an Next.js-Code immer die passende Doku in `node_modules/next/dist/docs/` pr√ºfen, weil diese Next.js-Version von √ºblichen Annahmen abweichen kann.
...
- Login l√§uft √ºber Firebase Auth
- Rollen werden √ºber Firestore in `userProfiles/{uid}` gepflegt
...
- `Hinzuf√ºgen` im linken Men√º ist jetzt als aufklappbares Dropdown aufgebaut
- √úber `Hinzuf√ºgen` gibt es links jetzt zus√§tzlich einen Bestands-Reiter mit Hierarchie:
  - Firma
  - zugeh√∂rige Objekte
  - zugeh√∂rige Einheiten
  - aktueller Mieter plus letzte Mieter chronologisch darunter
- √úber dem Dashboard gibt es links jetzt ein globales Suchfeld f√ºr Firma, Objekt, Einheit und Mieter
- Ein Klick im Bestandsbaum √∂ffnet jetzt zus√§tzlich direkt die passende Ansicht:
  - Firma √∂ffnet Firmenansicht
  - Objekt √∂ffnet Immobilienansicht
  - Einheit √∂ffnet die Immobilie mit Fokus auf die gew√§hlte Einheit
  - aktueller oder fr√ºherer Mieter √∂ffnet die Mieteransicht
...
- Formulare f√ºr Firmen, Personen und Immobilien wurden deutlich erweitert
- Uploadbereiche sind strukturell vorbereitet, speichern aktuell aber zun√§chst Dateinamen statt echter Storage-Dateien
- Firmenformular enth√§lt jetzt auch eine Steuerberater-Zuordnung per Dropdown
- Firmenbereich hat jetzt eine echte √úbersicht √ºber allen Firmen oberhalb des Anlegeformulars
- In der Firmen√ºbersicht gibt es pro Firma die Aktionen:
  - `Ansehen`
  - `Bearbeiten`
  - `L√∂schen`
- Detailseite f√ºr Firmen unter `/admin/firma/[id]` vorhanden
- Bearbeitungsseite f√ºr Firmen unter `/admin/firma/[id]/bearbeiten` vorhanden
- Detailseite f√ºr Immobilien unter `/admin/immobilie/[id]` vorhanden
- Detailseite f√ºr Mieter unter `/admin/mieter/[id]` vorhanden
- Firmen√ºbersicht zeigt jetzt nur noch den Firmennamen mit den drei Aktionen
- Admin-Formulare f√ºr Firmen, Personen und Immobilien arbeiten jetzt mit gemeinsamer Adresshilfe:
  - Stra√üe wird separat von Hausnummer gef√ºhrt
  - PLZ wird formatiert
  - Ort und Land werden soweit m√∂glich aus vorhandenen Adressen erg√§nzt
- Admin-Felder formatieren Eingaben jetzt beim Ausf√ºllen und beim Speichern st√§rker automatisch
- Admin-Formulare bremsen Browser-Credential-Autofill jetzt st√§rker aus, damit Kontakt- oder Postfachfelder nicht als Login gespeichert werden
- Personenbereich ist jetzt fachlich st√§rker aufgeteilt in:
  - Mieter separat
  - externe Partner / Handwerker / Dienstleister
- Im Bereich `Dritte & Dienstleister` sind die ausw√§hlbaren Rollen jetzt deutlich konkreter:
  - Elektriker
  - Sanit√§r / Rohrreinigung
  - Heizungsdienst
  - M√ºllabfuhrunternehmen
...
- Das zus√§tzliche Feld `Adresse intelligent √ºbernehmen` wurde wieder entfernt
- √úbersichten in Mieter, Dritte & Dienstleister, Immobilien und Firmen sind jetzt kompakter und einheitlicher aufgebaut
- Verwalterbereich gestalterisch an Startseite angen√§hert
- Dashboard grob neu ausgerichtet auf:
  - neueste Mieterkommunikation
  - Tickets
  - Leerstand an Einheiten
  - Online-Status und sp√§tere Historie von Mietern
...
- Eigent√ºmer werden √ºber ein einziges Feld aus `companies` gew√§hlt
- `Verwalter` erscheint nur bei `Teileigentum`
- Einheiten werden direkt im Objekt gepflegt:
  - Lage / Geb√§udeteil
  - Geschoss
  - Maisonette als zus√§tzliche Geschoss-/Lageoption
  - Positionsangaben `li`, `mi`, `re`
  - Mieterzuordnung
  - wohnungsbezogene Z√§hler als optionale, einzeln hinzuf√ºgbare Bausteine
  - vorbereitete Dokument- und Bildfelder
- Leerstand wird dort automatisch aus Einheiten ohne Mieterzuordnung abgeleitet
- Objekt-Z√§hler und Dienstleister-Zuordnungen sind jetzt als eigene Bereiche vorbereitet
- Objekt-Z√§hler werden jetzt ebenfalls nur bei Bedarf hinzugef√ºgt statt als starre Feldliste
- Teilungserkl√§rung liegt jetzt auf Objektebene
- √úbergabeprotokolle sind aus der Einheit entfernt und f√ºr die sp√§tere Mieterlogik vorgesehen
- Kaufdatum, Eigentum seit, Kaufpreis und Anfangsrendite sind im Immobilienformular erg√§nzt
- Mieter werden jetzt im Adminbereich nicht mehr per Freitext an Objekt / Einheit geh√§ngt, sondern aus bestehenden Einheiten ausgew√§hlt
- Die Einheit ist damit die fachliche Basis f√ºr Mietzuordnung und Leerstandslogik
- Beim Anlegen eines Mieters wird die gew√§hlte Einheit jetzt direkt im Objekt als belegt zur√ºckgeschrieben
- Das Mieterformular enth√§lt jetzt zus√§tzlich:
  - Steuernummer
  - B√ºrgschaft
  ...
  - Mieterh√∂hungsart und n√§chstes Pr√ºfdatum
  - vorbereitete Dokumentfelder f√ºr Mietvertrag, Nachtr√§ge, Ausweiskopien, SCHUFA, Gehaltsnachweise, Jahresabrechnungen u. a.
- Bei Immobilien sind zus√§tzlich Felder f√ºr:
  - Baujahr Heizung
  - letzte Heizungswartung
  - letzte Dachwartung
  - letzte Regenrinnenreinigung
  vorbereitet
- Bei Immobilien k√∂nnen jetzt mehrere Heizungsarten pro Objekt hinterlegt werden
- Kaufpreis wird im Immobilienformular jetzt als Geldbetrag formatiert
- Neue Einheiten werden im Immobilienformular jetzt direkt oberhalb der bestehenden Einheiten eingef√ºgt
- Objekt- und Einheitenz√§hler werden jetzt nur bei Bedarf hinzugef√ºgt und enthalten jeweils:
  - Z√§hlernummer
  - erster Z√§hlerstand
  - Ablesedatum
  - Eichdatum
- Die gro√üe Einleitungsbox oberhalb des Immobilienformulars wurde entfernt
...
- Admin darf fremde Profile lesen / √§ndern / l√∂schen
- `people`, `properties`, `companies`, `tenants` nur f√ºr Admin
- sonst alles standardm√§√üig gesperrt
...
- F√ºr sp√§tere Mieterfunktionen im Portal oder in der App m√ºssen die Regeln gezielt erweitert werden
...
- Claim aktuell:
  - `Family Office ‚Ä¢ Verm√∂gensmanagement`
- Haupt√ºberschrift:
...
- zus√§tzlicher `Home`-Button zur√ºck zur Startseite
- Formulartext oberhalb des Logins entfernt
- Submit-Button hei√üt nur noch `Anmelden`

---

## N√§chster sinnvoller Fokus

Der aktuell beste n√§chste Schritt ist:

### Verwalterbereich funktional ausbauen

Empfohlene Reihenfolge:

1. Datens√§tze nicht nur anlegen, sondern auch bearbeiten und l√∂schen
2. Detailfelder f√ºr Personen, Firmen und Immobilien erweitern
3. Beziehungen und Zuordnungen sauber ausbauen
4. √úbersichten mit Suche / Filtern erg√§nzen
5. sp√§tere Vorg√§nge vorbereiten:
   - Schadensmeldungen
   - Z√§hlerst√§nde
   - Dokumente
   - Zust√§ndigkeiten
...
3. Datenmodell, Rechte und Abl√§ufe festziehen
...
- So vermeiden wir doppelte Entscheidungen und sp√§tere Umbauten
...
- `components/admin/companyConfig.ts`  
  zentrale Felddefinitionen f√ºr Firmen
...
- `firestore.rules`  
  Sicherheitsregeln f√ºr Firestore
...
**Auth:** funktionsf√§hig  
**Rollentrennung:** funktionsf√§hig  
**Verwalterbereich:** Grundstruktur vorhanden, Ausbau als n√§chster Fokus  
...
- Die bisherigen Platzhalterseiten `Nachrichten` und `Tickets` wurden als erste echte Arbeitsoberfl√§chen aufgebaut
- Neue Admin-Seiten:
  - `/admin/nachrichten`
  - `/admin/tickets`
...
- Mieter k√∂nnen dort:
  - frei schreiben
  - optional eine grobe Kategorie ausw√§hlen
  - ihre bisherigen Nachrichten im Portalverlauf sehen
- Zus√§tzlich gibt es jetzt einen serverseitigen E-Mail-Eingang unter `/api/inbound-email`
- Feste Empf√§ngeradresse daf√ºr:
  - `portal@halbmann-holding.de`
- Der Endpoint ist daf√ºr gedacht, sp√§ter von einem Mailanbieter / Inbound-Webhook bedient zu werden
- Zus√§tzlich gibt es jetzt einen serverseitigen IMAP-Sync unter `/api/inbound-email/sync`
- Der IONOS-Posteingang ist lokal vorbereitet √ºber:
  - `imap.ionos.de`
  - Port `993`
...
- Der IMAP-Sync l√§uft jetzt automatisch an, sobald ein Admin den gesch√ºtzten Bereich √∂ffnet
- Pro Admin-Session wird der automatische Sync nur einmal ausgel√∂st
- Es gibt jetzt zus√§tzlich einen serverseitigen SMTP-Versand f√ºr vorbereitete Entw√ºrfe
- Neue Serverroute daf√ºr:
  - `/api/message-drafts/send`
- Entw√ºrfe k√∂nnen jetzt aus:
  - `Nachrichten`
  - `Tickets`
  direkt versendet werden
- Beim Versand passiert jetzt serverseitig:
  - SMTP-Versand √ºber den IONOS-Account
  ...
  - am Ticket wird ein `ticketEvent` f√ºr den Versand angelegt
...
- Firestore Rules erlauben Mietern jetzt:
  - Lesen des eigenen `tenant`-Datensatzes √ºber die eigene E-Mail
  - Lesen der eigenen `messages`
  - Anlegen eigener Portal-Nachrichten
  - Lesen eigener `tickets`
- `Nachrichten` enth√§lt jetzt:
  - Inbox mit Statusfiltern
  - Originalnachricht
  - automatische Analyse
  - Zuordnungsvorschlag f√ºr Mieter, Objekt, Einheit und Gewerk
  - vorbereitete Entw√ºrfe
- `Tickets` enth√§lt jetzt:
  - Ticketliste
  - Statuswechsel
  - verkn√ºpfte Ursprungsnachricht
  - verkn√ºpfte Entw√ºrfe
  - Timeline √ºber `ticketEvents`
- Neue Collections daf√ºr:
  - `messages`
  - `tickets`
  - `ticketEvents`
  - `messageDrafts`
...
- Neue Hilfslogik in `lib/adminWorkflow.ts`:
  ...
  - Priorit√§tserkennung
  - Zuordnung √ºber bekannte Mieter, Objekte und Einheiten
  - Vorschlag passender Dienstleister √ºber Objekt-Zuordnungen
  - Entwurf f√ºr Antwort an Mieter
  - Entwurf f√ºr Nachricht an Handwerker / Dienstleister
- Neue Server-Hilfsdateien:
  ...
  - `lib/imapSync.ts`
  - `lib/smtp.ts`
- Firestore Rules wurden f√ºr diese neuen Collections erweitert
- Der Gesamt-Build ist mit diesem Stand gr√ºn
- Die gro√üen Einleitungsbl√∂cke auf:
  - `Dashboard`
  - `Nachrichten`
  - `Tickets`
  wurden entfernt
- Titel und Beschreibung dieser Bereiche liegen jetzt im gemeinsamen Header des gesch√ºtzten Layouts statt doppelt in den Seiten selbst
- Im Admin-Header gibt es jetzt ein globales Einstellungsmen√º √ºber ein Zahnrad
- Erste Einstellungsseite:
  - `/admin/einstellungen`
- Dort kann das globale Mail-Postfach jetzt gepflegt, deaktiviert oder komplett gel√∂scht werden:
  - E-Mail-Adresse
  - IMAP Host / Port / Benutzer / Passwort
  - SMTP Host / Port / Benutzer / Passwort
- Die Serverlogik f√ºr IMAP-Sync und SMTP-Versand liest die Mailbox-Konfiguration jetzt zuerst aus `adminSettings/mailbox`
- ENV-Werte bleiben als Fallback f√ºr die Entwicklung erhalten
- Neue Collection:
  - `adminSettings`
- Firestore Rules wurden daf√ºr auf Adminzugriff erweitert

N√§chster sinnvoller Ausbau in diesem Bereich:

1. Firebase-Admin-Zugang lokal / auf dem Server vollst√§ndig hinterlegen, damit die Serverrouten produktiv schreiben k√∂nnen
2. IMAP-Sync einmal gegen das echte IONOS-Postfach testen
3. Vorlagenbereich f√ºr Standardschreiben aufbauen
4. manuelle Korrektur der KI-Zuordnung in `Nachrichten` erweitern
5. Anh√§nge und echte Antwortketten in Ein- und Ausgang erg√§nzen

### Mieter

- Mieter werden √ºber bestehende freie Einheiten angelegt
- Die Warmmiete wird jetzt automatisch aus `Kaltmiete + Betriebskosten` gebildet
- Kaution hat jetzt die Arten:
  - `Barkaution`
  - `Bankb√ºrgschaft`
- B√ºrge wird nicht mehr frei eingetippt, sondern aus der Personenliste gew√§hlt
- Daf√ºr gibt es bei `Dritte & Dienstleister` jetzt zus√§tzlich die Kategorie `B√ºrge`
- Mieterh√∂hung ist fachlich vorbereitet:
  - `Staffelmiete` mit wiederholbaren Zeilen
  - je Zeile: `von`, `bis`, `Kaltmiete`, `Erh√∂hung %`, `Erh√∂hung EUR`
  - Prozent und Euro rechnen sich gegenseitig
  - Erinnerungslogik:
    - Staffel: 1 Monat vor der n√§chsten Staffel
    - Index: j√§hrlich mit 1 Monat Puffer
    - Gesetz: alle 3 Jahre mit 1 Monat Puffer

### Immobilien

- Bei Immobilien gibt es jetzt einen eigenen Block `J√§hrliche Wartungen`
- Dort stehen untereinander:
  - `Heizung`
  - `Dach`
  - `Regenrinnenreinigung`
- F√ºr alle Wartungen wird das letzte Wartungsdatum gepflegt
- Fachliche Zielrichtung: sp√§tere Ticket-Erinnerung nach 11 Monaten
...
- `Ansehen / Bearbeiten / L√∂schen` ist jetzt √ºbergreifend f√ºr Firmen, Immobilien, Mieter und Dritte & Dienstleister ausgebaut
- Beim L√∂schen eines Mieters wird die Zuordnung in der betroffenen Einheit wieder entfernt

- Das Immobilienformular startet jetzt ohne vorgelagerten Einleitungsblock direkt im Formular
- Die Umsatzsteuer im Mieterformular ist neutral als Regelung f√ºr die Nettomiete modelliert und nicht mehr falsch an die Warmmiete gekoppelt

### Z√É¬§hler und Heizungen

- Immobilien zeigen jetzt eine kompakte Z√É¬§hler-Tabelle mit `Z√É¬§hler`, `Z√É¬§hlernummer`, `Eichdatum` und `Position`
- Es gibt eine eigene Z√É¬§hler-Detailseite mit Historie, neuen Z√É¬§hlerst√É¬§nden und dokumentiertem Z√É¬§hlerwechsel
- Z√É¬§hler haben im Formular jetzt zus√É¬§tzlich das Feld `Position`
- Heizungen werden auf Objektebene jetzt pro Heizsystem mit eigener Wartung und eigenem Baujahr gepflegt
- Falls keine Zentralheizung vorhanden ist, k√É¬∂nnen Heizungen auch auf Einheitenebene gepflegt werden

### Men√É¬º

- Im Admin-Men√É¬º gibt es jetzt zus√É¬§tzlich die Bereiche `Nachrichten` und `Tickets` als vorbereitete Platzhalterseiten

### Admin-Einstellungen / Mailbox

- Im Admin-Header gibt es jetzt ein globales Zahnrad-Men¸
- Darin gibt es eine Einstiegsseite f¸r das globale E-Mail-Postfach unter `/admin/einstellungen`
- Die Mailbox-Konfiguration wird nicht mehr direkt aus dem Browser nach Firestore geschrieben, sondern ¸ber die Serverroute `/api/admin/mailbox-settings`
- Dadurch sind Speichern und Lˆschen stabiler und nicht mehr von clientseitigen Firestore-Regeln abh‰ngig
- Das Lˆschen setzt die Mailbox bewusst auf inaktiv und leer, damit nicht ungewollt eine alte Fallback-Konfiguration aktiv bleibt
- In den Passwortfeldern f¸r IMAP und SMTP gibt es jetzt `Anzeigen` / `Verbergen`
- `npm run build` l‰uft nach diesem Umbau gr¸n
- F¸r `/admin/einstellungen` gibt es jetzt zus‰tzlich einen lokalen Dev-Fallback ohne Firebase-Admin-Credentials
- Wenn lokal keine Admin-Creds gesetzt sind, speichert die Mailbox-Seite in `.mailbox-settings.local.json`
- Die Serverlogik f¸r IMAP/SMTP liest diese lokale Mailbox-Datei vor dem ENV-Fallback
- Passwortfelder f¸r IMAP und SMTP haben jetzt `Anzeigen` / `Verbergen`
- `npm run build` bleibt nach diesem Fallback gr¸n
...
- Aus einer eingehenden Nachricht werden jetzt automatisch vorbereitet:
  ...
  - Entwurf f¸r das passende Gewerk / den Dienstleister
- E-Mail-Import ruft den Auto-Workflow jetzt direkt nach dem Speichern der Nachricht auf
- Das Mieterportal schreibt Nachrichten nicht mehr direkt in Firestore, sondern ¸ber `/api/portal/messages`
- Auch Portal-Nachrichten bekommen dadurch sofort Ticket und Entw¸rfe
- Neue Route: `/api/portal/messages`
- `npm run build` l‰uft nach diesem Ausbau gr¸n
...
- Wenn lokal kein Firebase-Admin vorhanden ist, liefert der IMAP-Sync die eingegangenen Mails an den Browser zur¸ck und der Admin-Client schreibt sie selbst in Firestore
...
- Der automatische Mail-Sync l‰uft nicht mehr nur einmal pro Session, sondern beim Einstieg und danach alle 30 Sekunden erneut
- Wenn kein Firebase-Admin konfiguriert ist, holt der Server die E-Mails per IMAP und der Admin-Client importiert sie anschlieﬂend selbst in Firestore
- Damit ist der lokale Entwicklungsmodus f¸r echten E-Mail-Eingang ohne Service-Account vorbereitet
- `npm run build` l‰uft nach diesem Umbau gr¸n
- Im lokalen Dev-Modus markiert der IMAP-Sync Mails nicht mehr vorschnell als `gesehen`
- Stattdessen werden die letzten bis zu 20 Mails regelm‰ﬂig erneut gepr¸ft und ¸ber `messageId` dedupliziert importiert
- So kˆnnen zuvor verpasste oder bereits einmal gesehene Testmails im lokalen Workflow trotzdem noch in `messages` auftauchen
- `npm run build` l‰uft nach dieser ƒnderung gr¸n
...
## Update 2026-04-03
- `Nachrichten` wurde weiter vereinfacht: sichtbare Entw¸rfe sind aus dem UI entfernt, `Posteingang` zeigt nur offene eingehende Nachrichten, `ticket_created` und `done` verschwinden aus der Inbox.
- `Mail senden` wurde neu gewichtet: Empf‰ngerbereich schmaler, Editor grˆﬂer, bei `Einzelne Empf‰nger` jetzt Auswahl `Immobilie -> Mieter` plus manuelle E-Mail und Vorlagenwahl.
- Nachrichtendetail wurde gestrafft: kompakte Zuordnungsinfos oben, kein separates Zuordnungsfeld mehr, keine Buttons f¸r Antwortentwurf/Dienstleister/Lˆschen mehr. Stattdessen direkter Antwortbereich mit Vorlagenwahl, `Ticket erstellen` und `Als erledigt markieren` bzw. R¸ckg‰ngig.
- `Bestand > Mieter` zeigt jetzt zus‰tzlich den Nachrichten- bzw. Chatverlauf des Mieters mit Link in die jeweilige Nachricht.
- Globale Einstellungen wurden erweitert: `/admin/einstellungen` hat jetzt Reiter f¸r `E-Mail-Postfach` und `Signaturen`; im Zahnrad-Men¸ im Admin-Header gibt es jetzt auch den Link `Signaturen`.
- Signaturen bleiben pro Firma pflegbar, inklusive Logo-Upload ¸ber Firebase Storage.
- `Tickets` zeigt keine sichtbaren Entwurfsblˆcke mehr.
...
- `Mail senden` wurde weiter verschoben: Empf‰ngerbereich schmaler, Editor breiter.
- `Tickets` wurde neu geordnet: keine zusammenfallenden schmalen Spalten mehr, klare Zweiteilung aus Liste und Detail, Ticket-Lˆschen erg‰nzt.
- `storage.rules` und `firebase.json` um Storage-Regeln erg‰nzt. Beim Deploy zeigte sich: Firebase Storage ist im Projekt `halbmann-app` noch nicht initialisiert. F¸r echtes Bucket-Hosting muss Storage in der Firebase-Konsole einmal aktiviert werden.
- Verifiziert mit `npm run build`.

## Update 2026-04-03 ñ Signaturen, Mail senden und Tickets
- Signaturen wurden fachlich erweitert: pro Firma jetzt mit `Name`, `Firmenname`, `Straﬂe`, `Hausnummer`, `PLZ`, `Ort`, `Mobilfunk`, `Telefon`, `E-Mail`, `Website` und Logo.
- Die Signaturfelder werden jetzt aus vorhandenen Firmendaten vorausgef¸llt und bleiben anschlieﬂend manuell anpassbar.
- Die Signaturvorschau wurde optisch gestrafft: Name ¸ber Firmenname, Anschrift integriert, Kontaktblock kleiner und enger gesetzt.
- Der bisher h‰ngende Logo-Upload wurde von Firebase Storage entkoppelt und l‰uft jetzt lokal ¸ber `/api/admin/signature-logo` direkt in `public/uploads/signatures`.
- `Mail senden` nutzt jetzt die gew‰hlte Firmen-/Mieter-Zuordnung f¸r die Signatur automatisch mit.
- Bei Einzelversand wird die passende Signatur direkt in den Editor eingesetzt.
- Beim Sammelversand an mehrere Mieter wird die Signatur pro Empf‰nger anhand der zugehˆrigen Firma erzeugt.
- Das Empf‰ngerfeld in `Mail senden` wurde weiter verkleinert, der Editorbereich entsprechend vergrˆﬂert.
- Eine Nachricht kann jetzt mehrere Tickets erzeugen; daf¸r werden zus‰tzliche Ticket-IDs an der Nachricht mitgef¸hrt.
- `Tickets` wurde auf dieselbe Arbeitslogik wie `Nachrichten` umgestellt:
  ...
  - Klick auf ein Ticket ˆffnet `/admin/tickets/[ticketId]`
- Die neue Ticket-Detailseite enth‰lt jetzt:
  - Verlauf aus verkn¸pften Nachrichten und Ticket-Events
  - manuellen Verlaufsblock
  - Mieter kontaktieren
  - Gewerk kontaktieren
  - Gewerk-Vorschlag aus der App, aber mit freier Auswahl aus allen hinterlegten Gewerken
  - Statuswechsel und Ticket-Lˆschen
...
## Update 2026-04-03 ñ Ticket-Detailseite und professionelle Signaturen
- Die Ticket-Detailseite wurde erneut neu geordnet und st‰rker an einen echten Arbeitschat angelehnt.
- Oben steht jetzt nur noch der Ticketkopf mit Ticketnummer sowie Firma, Objekt, Einheit und Mieter in einer kompakten Zeile.
- Der fr¸here breite Info-Block unter dem Ticketkopf wurde entfernt.
- Der Status wird jetzt prim‰r ¸ber hervorgehobene Status-Buttons gesteuert statt ¸ber einen separaten Statusblock.
- Tickets haben jetzt zus‰tzlich eine echte Wiedervorlage ¸ber `followUpDate`.
- Der Verlauf auf der Ticketseite f¸hrt jetzt alles zusammen:
  - Nachricht vom Mieter
  - Antworten an den Mieter
  - Nachrichten an Gewerke
  - interne Notizen
  - sonstige Ticket-Events
- Diese Verlaufseintr‰ge sind farblich und inhaltlich voneinander getrennt, damit klar bleibt, was Mieterchat ist und was interner bzw. Gewerk-Verlauf.
- Die urspr¸ngliche Nachricht des Mieters steht jetzt nicht mehr separat als freier Textblock im Kopf, sondern im eigentlichen Verlauf.
- Das bisher getrennte Feld `Mieter kontaktieren`, das Feld `Gewerk kontaktieren` und das Feld `Verlauf manuell erg‰nzen` wurden in einen gemeinsamen Eingabebereich ¸berf¸hrt.
- Dort kann jetzt zwischen drei Modi gewechselt werden:
  - `Mieter`
  - `Gewerk`
  - `Notiz`
- F¸r `Gewerk` gibt es eine Auswahl aller hinterlegten Gewerke am Objekt; der von der App erkannte Vorschlag wird vorausgew‰hlt.
- Nachrichten an den Mieter werden weiter als regul‰re ausgehende Nachrichten gespeichert und erscheinen dadurch sowohl im Ticketverlauf als auch im Nachrichtenverlauf des Mieters.
- Nachrichten an Gewerke werden ebenfalls aus dem Ticket verschickt, sind aber im Verlauf als eigener Typ gekennzeichnet.
- Tickets bleiben lˆschbar.
- Signaturen wurden fachlich deutlich erweitert:
  - zus‰tzlich zu Name, Firma, Anschrift, Mobilfunk, Telefon, E-Mail und Website jetzt auch Rechtsform, Abteilung/Zusatz, Registergericht, Handelsregister, Steuernummer und USt-IdNr.
- Diese Signaturfelder werden beim ÷ffnen einer Firma aus vorhandenen Firmendaten vorausgef¸llt.
...
- Der Logo-Upload l‰uft weiter lokal ¸ber `/api/admin/signature-logo` und legt Dateien in `public/uploads/signatures` ab.
- Der Versandpfad schreibt jetzt bei ausgehenden Nachrichten zus‰tzlich `draftKind`, `recipientType` und `recipientId` mit, damit Ticket- und Nachrichtenverl‰ufe sauber unterscheiden kˆnnen, ob eine Nachricht an Mieter oder Gewerk ging.
- Verifiziert mit `npm run build` in `C:\Users\simon\Documents\halbmann-app`.
...
- Wiedervorlage sitzt jetzt direkt bei den Status-/Lˆsch-Buttons im Kopf.
...
- Einheit wird lesbar aufgelˆst statt UUID-Fallback, bevorzugt aus 	enant.unitLabel bzw. property.units mit kompakter Floor/Positionslogik.
...
- Signaturen erweitert: zus‰tzliche Felder Sitz der Gesellschaft und Gesch‰ftsf¸hrung in lib/signatures.ts und components/admin/AdminSignatureSettings.tsx.
- Signaturvorschau dichter gestaltet, Kontaktdaten kleiner gesetzt, Logo in der Vorschau deutlich grˆﬂer.
- 
pm run build l‰uft gr¸n.

## Update 2026-04-03 21:28
- Ticket-Detailseite erneut verdichtet und funktional angepasst.
- Ticketkopf jetzt mit editierbarem Titel, kompakter Metazeile (Firma, Objekt, Einheit, Mieter) und nur noch Status In Bearbeitung / Erledigt plus Ticket lˆschen und Wiedervorlage.
- Ticket lˆschen fragt jetzt per Best‰tigungsdialog nach und leitet danach zur Ticketliste zur¸ck.
- Composer im Ticket steht jetzt ¸ber dem Verlauf; Verlauf sortiert neueste Nachricht zuerst.
- Tenant-/Gewerk-Nachrichten und Notizen laufen im Verlauf mit klar unterschiedlichen Karten; doppelte System-Vorg‰nge bleiben ausgeblendet.
- Antwortentw¸rfe an Mieter verwenden jetzt bevorzugt den erkannten Mieter statt romName.
- Gewerk-Composer nutzt im Editor die kleine Signatur; E-Mail-Versand erh‰lt zus‰tzlich HTML mit kompaktem Signaturblock plus Footer.
- Signaturen erweitert um Sitz der Gesellschaft und Gesch‰ftsf¸hrung; Vorschau dichter und Logo grˆﬂer.
...
- Gewerk-Entw¸rfe nutzen jetzt einen stabileren Empf‰ngernamen-Fallback (Person, Firma oder E-Mail), damit nicht nur Guten Tag, erscheint.
- Ticket-Textarea setzt jetzt lang=de und spellCheck=false, damit die rote Browser-Unterstreichung im Editor verschwindet.
- HTML-Mail-Signatur im Versandpfad optisch verbessert: oberer Signaturblock kompakt, restliche Pflichtangaben zentriert in mehreren Spalten statt als lange Einzels‰ule.
- 
pm run build bleibt gr¸n.

## Update 2026-04-04 00:06
- OpenAI-Anbindung f¸r individuelle Ticket-Antworten vorbereitet.
- Neue Serverroute: pp/api/ai/ticket-draft/route.ts.
- Nutzt OpenAI Responses API ¸ber das offizielle openai SDK.
- Ticket-Composer hat jetzt einen Button KI-Entwurf f¸r die Modi Mieter und Gewerk; Notiz bleibt manuell.
...
- F¸r produktive Nutzung muss OPENAI_API_KEY in .env.local gesetzt sein; optional OPENAI_MODEL (Fallback aktuell gpt-5-mini).
- 
pm run build l‰uft gr¸n.
[2026-04-04T10:35:00+02:00] KI- und Kommunikations-Feinschliff weitergezogen: Im globalen Zahnrad-Men¸ gibt es jetzt einen direkten Link zu /admin/einstellungen?tab=ki. F¸r Mehrfach-Themen in einer Eingangsnachricht existiert nun /api/ai/message-ticket-suggestions; die Nachrichtendetailseite nutzt lokale Fallback-Vorschl‰ge plus KI-Titelvorschl‰ge, zeigt sie als ausw‰hlbare Chips und speichert beim Ticket-Anlegen zus‰tzlich issueFocus. Ticket-KI liest jetzt diesen issueFocus statt pauschal die gesamte Ursprungsnachricht. In Nachrichten-Details wurden Ticket-Hinweis, KI-Titel/Anwenden sowie grˆﬂere Chat-/Antwort-/Notizfl‰chen erg‰nzt. In Tickets wurden Composer und Verlauf deutlich vergrˆﬂert und beim KI-Hinweis ein zus‰tzlicher Anwenden-Button erg‰nzt. Im Mieterbereich wurde der Chatverlauf auf ein nachrichten‰hnliches Muster erweitert: eigener Composer mit KI-Entwurf, grˆﬂere Eingabefl‰che, Scrollbereich f¸r den Verlauf und direkte Antworten an genau diesen Mieter. Mail senden in Nachrichten wurde weiter vergrˆﬂert. Build ist gr¸n.

## Update 2026-04-04 12:03
- Nachrichten: Posteingang zeigt keine Endlˆschung mehr. Alte Nachrichten bietet jetzt Endg¸ltig lˆschen nur dort an.
- Tickets: Filter Gelˆscht bleibt aktiv. Soft-Delete l‰uft ¸ber Ticketstatus deleted, endg¸ltige Lˆschung nur in der Gelˆscht-Ansicht.
- Dritte & Dienstleister: Chatbereich in PersonDetailView ist aktiv. Ausgehende Gewerk-Nachrichten aus Tickets landen ¸ber messages.recipientType = contact und ecipientId im Kontakt-Chat, w‰hrend sie im Ticketverlauf weiterhin sichtbar bleiben.
- Encoding-Sch‰den in den zuletzt bearbeiteten Admin-Dateien bereinigt; 
pm run build ist wieder gr¸n.

## Update 2026-04-04 12:42
- Tickets: Composer f¸llt sich nicht mehr automatisch bei Mieter/Gewerk. Inhalt entsteht erst durch Freitext oder KI-Entwurf.
- Ticket-KI: Promptlogik auf Revision umgestellt. Zus‰tzliche Anweisung ¸berschreibt Standardformulierungen, bestehender Entwurf wird gezielt ¸berarbeitet.
- Nachrichten > Mail senden: l‰dt Absenderadresse aus den Mailbox-Einstellungen, zeigt sie im Formular an und gibt sie an die KI mit.
- Nachrichten > Mail senden: Z‰hlerauswahl f¸r den gew‰hlten Mieter / das gew‰hlte Objekt erg‰nzt; ausgew‰hlte Z‰hler gehen in den KI-Entwurf ein.
- Nachrichten > Mail senden: KI-Hinweis hat jetzt sichtbaren Anwenden-Button.
- Mieter und Dritte & Dienstleister: KI-Hinweis + Anwenden im Chatbereich erg‰nzt.
- Mail-Footer: kompakter auf maximal drei zentrierte Zeilen reduziert.
- message-drafts/send: nutzt f¸r gespeicherte ausgehende Nachrichten jetzt die konfigurierte Mailbox-Adresse als romEmail statt hartem Fallback.
- 
pm run build ist gr¸n.
[2026-04-04T11:34:00+02:00] UI/Workflow-Runde: TicketDetailWorkspace komplett neu aufgebaut (eigener Verlauf je Ticket plus gemeinsame Ursprungsnachricht, KI-Hinweis/Anwenden/Entwurf jetzt ¸ber dem Texteingabefeld, Composer leer bei Moduswechsel, Wiedervorlage/Status/Delete oben kompakt). TenantDetailView und PersonDetailView neu aufgebaut, Chatbereich jetzt ganz oben vor den Stammdaten. MessagesWorkspace: manuelle Z‰hlerauswahl entfernt, stattdessen automatische Z‰hler¸bergabe an die KI bei passenden Betreffen; permanentes Lˆschen alter Nachrichten schreibt jetzt Tombstones in deletedMessages. Mailbox-Einstellungen um mailHeaderText erweitert; ausgehende E-Mails erhalten jetzt einen gestalteten Header-Banner aus den Mail-Einstellungen. KI-Routen ticket-draft, message-draft, message-reply-draft vollst‰ndig in sauberem UTF-8 ersetzt und auf strengere Einzelthemen-/Anweisungslogik umgestellt. Firestore-Regeln erweitert um deletedMessages und live nach halbmann-app deployt. npm run build gr¸n.

[2026-04-04T14:10:00+02:00] Chat-/Mail-Logik erneut geh‰rtet: Mail-Import nutzt jetzt zus‰tzlich externalMessageKey (lib/mailIdentity.ts) f¸r Dedupe/Tombstones, sowohl serverseitig in lib/inboundEmailIngest.ts als auch im Client-Fallback in lib/clientWorkflow.ts. Alte, endg¸ltig gelˆschte Mails sollen dadurch nicht mehr wieder erscheinen, selbst wenn Message-IDs fehlen oder variieren. Outbound-Mails werden in app/api/message-drafts/send/route.ts jetzt immer als messages-Dokument gespeichert, auch ohne related messageId, und ¸bernehmen tenantId/propertyId/unitId/recipientType sauber aus dem Draft. Dadurch sollen ausgehende Nachrichten aus Mail senden k¸nftig auch im Mieter-/Dienstleister-Chat auftauchen. Der Sendepfad nutzt jetzt Signaturdaten aus dem Draft und baut HTML-Footer zentraler ¸ber lib/signatures.ts. KI-Routen message-reply-draft, message-draft und ticket-draft wurden erneut in sauberem UTF-8 ¸berarbeitet und um klaren Kontextmodus Neue Nachricht vs. Antwort auf Verlauf erweitert. TenantDetailView, PersonDetailView, TicketDetailWorkspace und MessageDetailWorkspace ¸bergeben diesen Modus jetzt an die KI bzw. setzen ihn im UI sichtbar. MessageDetailWorkspace wurde funktional aufger‰umt: direkte Antwort steht jetzt ¸ber dem Verlauf, Ticket-KI-Hinweis und KI-Titel-Button sind entfernt, stattdessen bleiben nur noch Vorschlags-Chips f¸r Mehrfachthemen-Tickets. TicketDetailWorkspace setzt Composer-Zustand beim Ticketwechsel zur¸ck, damit Inhalte nicht ins n‰chste Ticket mitwandern. PersonDetailView nutzt jetzt ebenfalls Firmen-Signaturdaten beim Versand. npm run build gr¸n.
[2026-04-04T15:55:00+02:00] Navigation/Signatur/Composer weiter bereinigt: In ProtectedAreaLayout wird der aktive Link jetzt als l‰ngster passender Treffer bestimmt, damit links nur noch genau eine Seite markiert bleibt. Neue Hilfslogik in lib/draftComposer.ts setzt Begr¸ﬂung und Portal-Signatur systemseitig, strippt KI-Begr¸ﬂung/Abschluss aus Antworten heraus und verhindert damit falsche Anreden oder Empf‰nger-Signaturen in TenantDetailView, PersonDetailView, TicketDetailWorkspace, MessageDetailWorkspace und MessagesWorkspace. lib/signatures.ts vollst‰ndig in sauberes UTF-8 ersetzt; E-Mail-Signatur besteht nun aus kurzem Abschlussblock (Mit freundlichen Gr¸ﬂen / Name / Firma) plus kompaktem, zentriertem Footer mit bis zu drei Zeilen und absolut aufgelˆstem Logo. app/api/message-drafts/send/route.ts ebenfalls in sauberem UTF-8 ersetzt und an die neue Abschluss-/Footer-Logik angepasst. buildTenantContact in lib/adminWorkflow.ts leitet Anreden nun zus‰tzlich aus anrede/gender ab. MessagesWorkspace zeigt Wiedervorlage jetzt nicht mehr nur im Einzelmodus, sondern ¸berall dort, wo Empf‰nger ausgew‰hlt sind. npm run build gr¸n.

[2026-04-04T16:35:00+02:00] Mail-/Ticket-Runde weiter bereinigt: draftComposer priorisiert jetzt eine im Kontext gefundene Anrede (z. B. Frau Tran) vor fehlerhaften Stammdaten-Anreden. app/api/ai/ticket-draft/route.ts wurde auf strikteren Einzelticket-Fokus geh‰rtet: andere Maengel aus derselben Ursprungsmail sollen explizit ignoriert werden, und Nutzerkorrekturen muessen Vorrang haben. AdminMailboxSettings wurde komplett in sauberes UTF-8 ersetzt und erlaubt jetzt getrennte Bearbeitung von Mail-Header und Mail-Footer mit benutzerfreundlichen Formatoptionen (Schriftart, Schriftgroesse, Ausrichtung, Fett, Kursiv, Unterstreichen, Trennlinie) neben den IMAP/SMTP-Daten. MailboxSettings/LocalConfig/ServerConfig/API wurden um diese Header-/Footer-Felder erweitert. app/api/message-drafts/send/route.ts rendert Header und Footer nun mit den gespeicherten Stiloptionen. Build ist gruen.

[2026-04-04T16:55:00+02:00] Navigations-/Ticket-/Settings-Runde: ProtectedAreaLayout weiter verdichtet (Bestand-Block optisch neutraler, Sidebar-Logo grˆﬂer). TicketsWorkspace nach oben gezogen, Filterbuttons und manueller Ticketbutton kompakter. TicketDetailWorkspace weiter nach oben gezogen; Zur¸ck-Link sitzt jetzt links oben im Content, Ticketkopf dichter in einer Zeile mit TK/Firma/Objekt/Einheit/Mieter. Einstellungen umgebaut: Tab Postfach heiﬂt nun Postfach-Zugang und zeigt nur noch Zugangsdaten; unter Signaturen werden jetzt AdminSignatureSettings plus die vollst‰ndige Header-/Footer-Steuerung aus AdminMailboxSettings angezeigt. Mailbox-Einstellungen unterst¸tzen getrennte Header-/Footer-Formatierung (Schriftart, Schriftgrˆﬂe, Ausrichtung, Fett, Kursiv, Unterstreichen, Trennlinie) serverseitig und lokal. buildIssueSuggestionsFromText in lib/adminWorkflow.ts erg‰nzt bekannte Problemvorschl‰ge priorisiert (u.a. Verstopfung in K¸che, Tropfende Toilette, Mangelhafte Treppenhausreinigung), damit Tickettitel aus Mehrfach-Nachrichten brauchbarer vorgeschlagen werden. npm run build gr¸n.
...
- `AdminMailboxSettings` unterst¸tzt jetzt die Modi `credentials`, `layout`, `full`.
- KI f¸r `Neue Nachricht` bei Mieter/Dienstleister h‰rter vom Verlauf getrennt: `message-reply-draft` ignoriert alte Themen in `new`-Kontext jetzt explizit, und Mieter sollen nicht mehr zur Firmen-Mailadresse geschickt werden.
- Sidebar `Bestand` optisch weiter neutralisiert, damit es nicht wie ein aktiver Link wirkt.
- Header/Settings-Men¸ mit hˆherem `z-index`, damit das Zahnrad nicht hinter Ticketfeldern verschwindet.
- Ticketliste und Ticketdetail weiter nach oben gezogen; Ticketdetail mit kompakterem Kopf und Back-Button oben links.
- Build gepr¸ft: `npm run build` gr¸n.
## Update 2026-04-04 17:40
- Ticket-/Ticketdetail-Headerleiste weiter vereinfacht: globaler Kopfstreifen f¸r `/admin/tickets` und `/admin/tickets/[ticketId]` entfernt, Settings-Zahnrad dort jetzt separat/floating.
- `AdminSignatureSettings` erweitert: Signatur hat jetzt eigenen Formatierungsblock mit Schriftart, Schriftgrˆﬂe, Ausrichtung, Fett, Kursiv, Unterstreichen und Trennlinie.
- Signaturdaten speichern jetzt zus‰tzliche Felder `signatureFontFamily`, `signatureFontSize`, `signatureTextAlign`, `signatureFontBold`, `signatureFontItalic`, `signatureFontUnderline`, `signatureUseDivider`.
- `lib/signatures.ts` aktualisiert, damit E-Mail-Signaturen diese Formatierung auch tats‰chlich im HTML-Footer ber¸cksichtigen.
...
- Ticketliste und Ticketdetail wurden weiter nach oben gezogen, damit die Kopfzeilen n√§her an die Zahnrad-Zeile r√ºcken.
- Die Ticketdetailseite nutzt jetzt eine kompaktere Metadatenzeile oben und einen zur√ºckgesetzten Arbeitsbereich darunter.
- Die Sidebar im Bereich Bestand wurde optisch entsch√§rft, damit nur noch echte aktive Eintr√§ge hervortreten.
- Im Mieterchat nutzt Neue Nachricht jetzt den KI-Pfad f√ºr eigenst√§ndige Nachrichten statt den Antwortpfad auf den Verlauf.
- Der KI-Kontext f√ºr Mieter ber√ºcksichtigt jetzt zus√§tzlich die konfigurierte Absenderadresse und vorhandene Z√§hlerdaten.
- Die KI-Regeln wurden versch√§rft, damit bei Tickets und Mieter-Nachrichten die Anrede Frau/Herr verbindlich ist und nur das aktuelle Einzelticket behandelt wird.
- matchCategory() priorisiert Hausreinigung und konkrete Problemf√§lle wie Treppenhausreinigung, tropfende Toilette und Verstopfung in K√ºche jetzt vor allgemeineren Treffern.
...
- 
pm run build ist auf diesem Stand gr√ºn.
...
- `app/admin/einstellungen/page.tsx` wurde daf¸r erweitert und ¸bergibt den aktiven Unterbereich an `components/admin/AdminLetterSettings.tsx`.
- `AdminLetterSettings` unterst¸tzt jetzt einen `view`-Prop:
  - `vorlage`
  - `anrede`
  - `abschluss`
...
- `Briefvorlage` zeigt weiterhin das groﬂe Vorlagenfeld mit Logo, Linien, R‰ndern und Platzhaltern.
- `Anrede` hat jetzt ein eigenes groﬂes Rich-Text-Feld nur f¸r die formatierbare Brief-Anrede.
- `Abschluss` hat jetzt ein eigenes groﬂes Rich-Text-Feld nur f¸r den formatierbaren Abschlussblock.
- Die Toolbar-Buttons wurden bereits so angepasst, dass ihre Auswahl per `selectionchange` zentraler gemerkt wird.
- Speichern der Rich-Text-Inhalte f¸r Anrede und Abschluss erfolgt nicht mehr bei jedem Tastendruck, sondern erst beim Verlassen des Feldes bzw. beim Speichern.
- `npm run build` lief nach dieser Umstellung erfolgreich gr¸n.

### Noch offener Punkt vor dem Neustart

- Die Trennung in drei Unterseiten ist jetzt drin, aber noch nicht abschlieﬂend im Browser verifiziert.
- Vor dem Neustart war genau das der n‰chste Pr¸fpunkt:
  - Funktioniert Formatieren in `Anrede` jetzt sauber?
  - Funktioniert Formatieren in `Abschluss` jetzt sauber?
  - Bleibt `Briefvorlage` stabil und arbeitet nur noch auf dem groﬂen Vorlagenfeld?

### Wichtige betroffene Dateien

- `app/admin/einstellungen/page.tsx`
- `components/admin/AdminLetterSettings.tsx`
- auﬂerdem weiter relevant f¸r Brief-/Signatur-Rendering:
  - `lib/signatures.ts`

### Hinweis f¸r den n‰chsten Chat
...
- Erster sinnvoller Schritt im neuen Chat:
  ...
  4. danach `Briefvorlage` kurz gegenpr¸fen
- Falls die Selektion dort immer noch springt, dann als n‰chstes nicht wieder symptomatisch patchen, sondern die Toolbar-/Selection-Logik in `AdminLetterSettings.tsx` f¸r die drei Views jeweils strikt getrennt behandeln.
...
- Alter Ticket-Zweig jetzt technisch deutlich h‰rter entfernt.
...
- In components/admin/TenantDetailView.tsx wird beim Splitten nicht mehr mit vielen ausw‰hlbaren Verlaufseintr‰gen gearbeitet.
...
- Verifiziert: `npm run build` gruen.
## 2026-05-05 10:05 - Portal-‹bersicht aufger‰umt
- `components/ProtectedAreaLayout.tsx`: F¸r `requiredRole="portal"` zeigt die linke obere Kachel jetzt nur noch den Portal-Namen (`profile.displayName` / Benutzername) ohne `Interner Bereich`. Der obere Header-Bereich wird f¸r Portal-Seiten komplett ausgeblendet.
- `app/mieterportal/page.tsx`: Die groﬂe Einstiegskachel mit `Portal`, Mieternamen und Erkl‰rungstext wurde entfernt.
- Verifiziert: `npm run build` gr¸n.
## 2026-05-05 10:28 - Portal-‹bersicht auf Mieterblick umgebaut
- `components/ProtectedAreaLayout.tsx`: Portal-Seitenleiste l‰dt jetzt den echten Portal-Anzeigenamen aus `/api/portal/context` und zeigt dadurch oben links den Mieternamen statt E-Mail/Benutzername.
- `app/mieterportal/page.tsx`: ‹bersicht komplett neu aufgebaut. Statt alter Kacheln jetzt Begr¸ﬂungstext, kompakte Mieterinformationen und ein Z‰hlerbereich mit aktuellem Stand pro Z‰hler.
- `app/api/portal/meters/route.ts`: Neuer Server-Endpunkt f¸r Z‰hlerstand-Meldungen aus dem Portal. Aktualisiert im Admin-Modus die Property/Unit-Meter in Firestore und im lokalen Portalmodus die hinterlegten lokalen Portaldaten.
- `app/api/portal/context/route.ts`: Lokaler Portal-Kontext liefert jetzt vorhandene `propertyData` aus `.portal-access.local.json` aus und unterst¸tzt Meterdaten in `targetData`/`unitMeters`.
...
- `lib/portalAccess.ts`: `cleanPortalText(...)` repariert jetzt haeufige Mojibake-Sequenzen (`Stra√üe`, `¬∑`, Umlaute usw.) direkt beim Lesen, damit alte lokale Portal-Datensaetze sauberer dargestellt werden.
- Verifiziert: `npm run build` gruen.
...
- `app/mieterportal/nachrichten/page.tsx`: Bei Rueckkehr mit `?sent=1` erscheint ein Best‰tigungs-Popup `Nachricht wurde verschickt.` und der Query-Parameter wird direkt wieder bereinigt.
...
- Konkrete Mieterseite im Nachrichtenmodus nutzt jetzt denselben Zweispalten-Grid wie der allgemeine Posteingang: die rechte Nachrichtenliste bleibt neben Composer/Arbeitsfl‰che und rutscht nicht mehr unter das Eingabefeld.
...
- Auf Mieterseiten ohne Kopf-Aktionen wird die leere obere Header-Zeile nicht mehr gerendert. Dadurch verschiebt die alte Negativmarge den Nachrichtenblock nicht mehr nach unten.
- Build-Blocker nebenbei bereinigt: `app/¸ber/page.tsx` war leer und wurde als gueltiges Modul erg‰nzt.
- In den Nachrichten-Infokacheln heisst `Einzug` jetzt `Mietbeginn`.
- Darunter wurden fuer beide Nachrichtenansichten `Mietende` und `Optionen` erg‰nzt. Wenn im Datensatz nichts vorhanden ist, erscheinen die Defaultwerte `Unbefristet` und `Keine Option hinterlegt`.
...
- `Bankbuergschaft` ist in der Nachrichten-Stammdatenkachel auf `Bankb¸rgschaft` korrigiert.
...
- Brief-Paginierung weiter robuster gemacht: Der Umbruch misst jetzt direkt die reale Scrollhoehe des Brief-Body-Bereichs statt Body- und Closing-Offsets manuell zusammenzurechnen. Das ist verl‰sslicher fuer den Fall, dass der Abschlussblock noch verdeckt unter der ersten Seite haengt.
- Der Briefmodus arbeitet jetzt ohne Seitenvorschau und Pagination. Stattdessen erscheint beim Versandmodus `Brief` ein direkt editierbares A4-Blatt mit sichtbarem Header, Empf‰ngerblock, Betreff, Footer und bearbeitbarem Body.
...
- 2026-05-06: Briefeditor-Slot im A4-Template stabilisiert: Der Vorlagenrahmen aus der Briefvorlage wird im Compose-Modus nicht mehr bei jeder Body-Aenderung neu aufgebaut. Der editierbare TipTap-Body aktualisiert jetzt den Inhalt, ohne den Body-Slot selbst zu zerstˆren. Das behebt den Fall, dass KI-Entwuerfe zwar erzeugt werden, aber im Briefblatt nicht erscheinen. Build gruen.

- 2026-05-06: Briefeditor-Body-Sync nachgezogen: externer Text (z. B. KI-Entwurf) wird nun gegen den aktuellen TipTap-Inhalt gepr¸ft und mit setContent direkt in den Body geschrieben, statt nur gegen einen internen Ref zu vergleichen. Das stabilisiert den Fall, dass KI-Entw¸rfe im Briefmodus erzeugt, aber nicht sichtbar wurden. Build gruen.
...
- 2026-06-02: DOCX-Vorlagenbefuellung nach Word-Test korrigiert: Mehrzeilige Werte wie {{EMPFAENGER_BLOCK}} und {{BETREFF_ZEILE_2}} werden jetzt mit echten Word-Zeilenumbruechen (w:br) ersetzt statt als zusammenlaufender Text. Zusaetzlich Platzhalter {{ABSCHLUSS}}, {{CLOSING}} und {{CLOSING_BLOCK}} ergaenzt; Abschluss kommt aus der Firmensignatur, Fallback Mit freundlichen Gr¸ﬂen. Build gruen mit npm.cmd run build.
