...
function normalizeAttachments(value: unknown): PortalAttachmentRecord[] {
  if (!Array.isArray(value)) return [];

  return value.reduce<PortalAttachmentRecord[]>((result, entry) => {
...
export async function POST(request: Request) {
  try {
    ...
    if (!hasFirebaseAdminConfig()) {
      ...
      const currentTheme = (await listLocalMessageThemes()).find((entry) => entry.id === themeId) ?? null;
      const now = new Date().toISOString();
      const nextMessageIds = Array.from(new Set([...(currentTheme?.messageIds ?? []), messageId]));
      ...
      return NextResponse.json({ ok: true, messageId, themeId, ticketId: '' });
    }
    ...
    const messagePayload = {
      ...
      fromName: buildPortalDisplayName(targetType, targetData),
      priority: 'normal',
      propertyId: cleanPortalText(targetData.propertyId),
